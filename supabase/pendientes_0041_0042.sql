...
--
--  Son las migraciones 0041 y 0042 juntas, en orden. Se pueden volver a
--  ejecutar sin romper nada: todo va con "create or replace" o
--  "if not exists".
--
--  0041 arregla que la lista de Plan dijera «con plan» de gente a la que ya
--  se lo habias quitado. El entrenador veia una cosa y el cliente otra.
--
--  0042 añade lo que hace falta para la ficha de «como va cada persona»:
--  la funcion que devuelve todas sus metricas y la tabla donde se guarda lo
--  que escribe la IA sobre ellas.
--
--  Al pegar, Supabase pedira confirmacion extra porque el texto contiene
--  "drop policy". Es normal: son las politicas que se vuelven a crear tres
--  lineas mas abajo.
--
--  COMPROBAR DESPUES (deben devolver algo, no error):
--    select nombre, tiene_plan from public.plan_lista();
--    select public.plan_metricas('<id-de-un-cliente-tuyo>');
-- ---------------------------------------------------------------------
